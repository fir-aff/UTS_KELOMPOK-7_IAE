...
class Menu(db.Model):
    ...
    def to_dict(self): return { 'id': self.id, 'name': self.name, 'price': self.price, 'restaurant_id': self.restaurant_id }
...
def get_all_restaurants():
    ...
    responses:
      200:
        description: List semua restoran
    """
...
def update_restaurant(id):
    ...
    responses:
      200:
        description: Restoran berhasil di-update
      404:
        description: Restoran tidak ditemukan
    """
    resto = Restaurant.query.get(id)
    if not resto: return jsonify({'error': 'Not found'}), 404
    data = request.get_json()
    resto.name = data.get('name', resto.name)
    resto.address = data.get('address', resto.address)
    db.session.commit()
    return jsonify({'message': 'Updated', 'restaurant': resto.to_dict()}), 200

@app.route('/restaurants/<int:id>', methods=['DELETE'])
def delete_restaurant(id):
    """
    Hapus restoran (dan semua menunya)
    ---
    tags:
      - Restaurant
    parameters:
    ...
    db.session.delete(resto); db.session.commit()
...
def add_menu_item(resto_id):
    """
    Menambah menu baru ke restoran
    ---
    tags:
      - Menu
    parameters:
    ...
    """
    data = request.get_json()
    new_menu = Menu(name=data['name'], price=data['price'], restaurant_id=resto_id)
    db.session.add(new_menu); db.session.commit()
    return jsonify({'message': 'Menu added', 'menu': new_menu.to_dict()}), 201
...
def get_menu_item(menu_id):
    ...
    return jsonify(menu.to_dict()) if menu else (jsonify({'error': 'Not found'}), 404)
...
def update_menu_item(menu_id):
    ...
    db.session.commit()
    return jsonify({'message': 'Updated', 'menu': menu.to_dict()}), 200
...
def delete_menu_item(menu_id):
    ...
    menu = Menu.query.get(menu_id)
    if not menu: return jsonify({'error': 'Not found'}), 404
    db.session.delete(menu); db.session.commit()
    return jsonify({'message': 'Deleted'}), 200

# --- HEALTH & SEED ---
# (Kode Health & Seed sama seperti sebelumnya, tidak perlu docs Swagger khusus)
